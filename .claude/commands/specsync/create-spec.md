...
5. Open the newly created `specs/<module-name>/<module-name>.spec.md` and fill
   in the required sections: `Purpose`, `Public API`, `Invariants`,
   `Behavioral Examples`, `Error Cases`, `Dependencies`, and `Change Log`.
   If a free-text description was given in step 2, use it directly to draft
   these sections — ask clarifying questions if it's underspecified, but do
   not leave sections as placeholder text. Do the same for `requirements.md`
